...
static void PrepareChunk(int x1, int y1, int z1) {
	...
	if (World.Type == WORLD_INFINITE) {
		xMax = x1 + CHUNK_SIZE; yMax = y1 + CHUNK_SIZE; zMax = z1 + CHUNK_SIZE;
	} else {
	...
	BlockID b;
	int x, y, z, xx, yy, zz;
	cc_bool infinite = World.Type == WORLD_INFINITE;
		...
		for (z = z1, zz = 0; z < zMax; z++, zz++) {
			...
			for (x = x1, xx = 0; x < xMax; x++, xx++, cIndex++) {
				...
				if (Builder_Counts[index] == 0 ||
					(!infinite && x == 0 && (y < Builder_SidesLevel || (b >= BLOCK_WATER && b <= BLOCK_STILL_LAVA && y < Builder_EdgeLevel))) ||
					((infinite || x != 0) && (Blocks.Hidden[tileIdx + Builder_Chunk[cIndex - 1]] & FACE_BIT_XMIN) != 0)) {
				...
				index++;
				if (Builder_Counts[index] == 0 ||
					(!infinite && x == World.MaxX && (y < Builder_SidesLevel || (b >= BLOCK_WATER && b <= BLOCK_STILL_LAVA && y < Builder_EdgeLevel))) ||
					((infinite || x != World.MaxX) && (Blocks.Hidden[tileIdx + Builder_Chunk[cIndex + 1]] & FACE_BIT_XMAX) != 0)) {
					Builder_Counts[index] = 0;
				} else {
				...
				index++;
				if (Builder_Counts[index] == 0 ||
					(!infinite && z == 0 && (y < Builder_SidesLevel || (b >= BLOCK_WATER && b <= BLOCK_STILL_LAVA && y < Builder_EdgeLevel))) ||
					((infinite || z != 0) && (Blocks.Hidden[tileIdx + Builder_Chunk[cIndex - EXTCHUNK_SIZE]] & FACE_BIT_ZMIN) != 0)) {
					Builder_Counts[index] = 0;
				} else {
				...
				index++;
				if (Builder_Counts[index] == 0 ||
					(!infinite && z == World.MaxZ && (y < Builder_SidesLevel || (b >= BLOCK_WATER && b <= BLOCK_STILL_LAVA && y < Builder_EdgeLevel))) ||
					((infinite || z != World.MaxZ) && (Blocks.Hidden[tileIdx + Builder_Chunk[cIndex + EXTCHUNK_SIZE]] & FACE_BIT_ZMAX) != 0)) {
					Builder_Counts[index] = 0;
				} else {
				...
				index++;
				if (Builder_Counts[index] == 0 || (!infinite && y == 0) ||
...
static cc_bool ReadChunkData(int x1, int y1, int z1, cc_bool* outAllAir) {
	...
	ReadChunkBody(World_GetBlock(x, y, z));
...
static cc_bool ReadBorderChunkData(int x1, int y1, int z1, cc_bool* outAllAir) {
	...
	ReadBorderChunkBody(World_GetBlock(x, y, z));
...
static void OutputChunkPartsMeta(int x, int y, int z, struct ChunkInfo* info) {
	...
	if (World.Type == WORLD_INFINITE) {
		struct ChunkPartInfo* nParts = info->normalParts;
		struct ChunkPartInfo* tParts = info->translucentParts;
		offset = 0;
		hasNorm = false;
		hasTran = false;
		...
		for (i = 0; i < MapRenderer_1DUsedCount; i++) {
			...
			hasNorm |= SetPartInfo(&Builder_Parts[i], &offset, &nParts[i]);
			hasTran |= SetPartInfo(&Builder_Parts[j], &offset, &tParts[i]);
		}
		return;
	}
...
cc_bool Builder_MakeChunk(struct ChunkInfo* info) {
	...
	if (World.Type == WORLD_INFINITE) {
		/* No map borders in an infinite world. */
	...
	if (World.Type == WORLD_INFINITE) {
		if (info->normalParts) {
			for (index = 0; index < MapRenderer_1DUsedCount; index++) {
				BuildPartVbs(&info->normalParts[index]);
			}
		}
		if (info->translucentParts) {
			for (index = 0; index < MapRenderer_1DUsedCount; index++) {
				BuildPartVbs(&info->translucentParts[index]);
			}
		}
	} else {
...
static PackedCol Normal_LightColor(int x, int y, int z, Face face, BlockID block) {
	...
	switch (face) {
	case FACE_XMIN:
		return (!infinite && x < offset)      ? Env.SunXSide : Lighting.Color_XSide_Fast(x - offset, y, z);
	case FACE_XMAX:
		return (!infinite && x > (World.MaxX - offset)) ? Env.SunXSide : Lighting.Color_XSide_Fast(x + offset, y, z);
	case FACE_ZMIN:
		return (!infinite && z < offset)      ? Env.SunZSide : Lighting.Color_ZSide_Fast(x, y, z - offset);
	case FACE_ZMAX:
		return (!infinite && z > (World.MaxZ - offset)) ? Env.SunZSide : Lighting.Color_ZSide_Fast(x, y, z + offset);
...
static void NormalBuilder_RenderBlock(int index, int x, int y, int z) {	
	...
	infinite   = World.Type == WORLD_INFINITE;
	...
	if (count_XMin) {
		...
		col = fullBright ? PACKEDCOL_WHITE :
			(!infinite && x < offset) ? Env.SunXSide : Lighting.Color_XSide_Fast(x - offset, y, z);
	...
	if (count_XMax) {
		...
		col = fullBright ? PACKEDCOL_WHITE :
			(!infinite && x > (World.MaxX - offset)) ? Env.SunXSide : Lighting.Color_XSide_Fast(x + offset, y, z);
	...
	if (count_ZMin) {
		...
		col = fullBright ? PACKEDCOL_WHITE :
			(!infinite && z < offset) ? Env.SunZSide : Lighting.Color_ZSide_Fast(x, y, z - offset);
	...
	if (count_ZMax) {
		...
		col = fullBright ? PACKEDCOL_WHITE :
			(!infinite && z > (World.MaxZ - offset)) ? Env.SunZSide : Lighting.Color_ZSide_Fast(x, y, z + offset);
...
void Builder_ApplyActive(void) {
	if (World.Type == WORLD_INFINITE) {
		/* Infinite worlds use the simple mesh builder, as smooth/advanced
		   lighting relies on whole-world state that does not exist here. */
