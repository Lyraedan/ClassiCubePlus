...
/* The view distance the user had set before an infinite world capped it. */
static int savedViewDist = -1;
/*########################################################################################################################*
*----------------------------------------------------------World----------------------------------------------------------*
*#########################################################################################################################*/
...
void World_Reset(void) {
	cc_bool wasInfinite = World.Type == WORLD_INFINITE;
	...
	World.Type     = WORLD_FINITE;

	/* Restore the user's view distance when leaving an infinite world. */
	if (wasInfinite && savedViewDist > 0) {
		Game_SetViewDistance(savedViewDist);
	}
	savedViewDist = -1;
...
void World_SetInfinite(int seed) {
	savedViewDist = Game_ViewDistance;
	World_NewMap();

	World.Type    = WORLD_INFINITE;
	World.Seed    = seed;
	World.Loaded  = true;

	ChunkStore_Init();
	ChunkStore_Generator = InfiniteGen_GenerateChunk;

	Env_Reset();
	if (Env.EdgeHeight == -1)   { Env.EdgeHeight   = 0; }
	if (Env.CloudsHeight == -1) { Env.CloudsHeight = 32; }

	/* Infinite worlds cannot preload a huge view distance. */
	if (Game_ViewDistance > WORLD_INF_MAX_VIEWDIST) {
		Game_SetViewDistance(WORLD_INF_MAX_VIEWDIST);
	}
...
void World_SetBlock(int x, int y, int z, BlockID block) {
	if (World.Type == WORLD_INFINITE) { ChunkStore_SetBlock(x, y, z, block); return; }
...
BlockID World_SafeGetBlock(int x, int y, int z) {
	if (World.Type == WORLD_INFINITE) return ChunkStore_GetBlock(x, y, z);
...
Vec3 Respawn_FindSpawnPosition(float x, float z, Vec3 modelSize) {
	...
	if (World.Type == WORLD_INFINITE) {
		/* Use the terrain surface height at the given column. */
		int surfaceY = InfiniteGen_GetSurfaceHeight((int)x, (int)z);
		Vec3_Set(spawn, x, (float)(surfaceY + ENTITY_ADJUSTMENT), z);
		AABB_Make(&bb, &spawn, &modelSize);

		for (y = surfaceY + 2; y >= surfaceY - 4; y--) {
