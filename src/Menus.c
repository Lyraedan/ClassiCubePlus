...
static struct GenLevelScreen {
	Screen_Body
	struct FontDesc textFont;
	struct ButtonWidget flatgrass, vanilla, infinite, cancel;
...
static void GenLevelScreen_ContextRecreated(void* screen) {
	...
	ButtonWidget_SetConst(&s->infinite,  "Infinite",           &titleFont);
...
static void GenLevelScreen_Layout(void* screen) {
	...
	Widget_SetLocation(&s->infinite,  ANCHOR_CENTRE, ANCHOR_CENTRE,    0,  100);
	Menu_LayoutBack(&s->cancel);
}
...
static void GenLevelScreen_Init(void* screen) {
	...
	ButtonWidget_Add(s, &s->infinite,  200, GenLevelScreen_Infinite);
...
static struct ClassicGenScreen {
	Screen_Body
	struct ButtonWidget btns[3], infinite, cancel;
	struct TextWidget title;
	struct Widget* __widgets[1 + 3 + 1 + 1];
} ClassicGenScreen;
...
static void ClassicGenScreen_Infinite(void* a, void* b) {
	RNGState rnd;
	int seed;
...
static void ClassicGenScreen_Layout(void* screen) {
	...
	Widget_SetLocation(&s->infinite,  ANCHOR_CENTRE, ANCHOR_CENTRE, 0,   50);
	Widget_SetLocation(&s->cancel,  ANCHOR_MAX,     ANCHOR_MAX,    0,   80);
}
...
static void ClassicGenScreen_Init(void* screen) {
	...
	ButtonWidget_Add(s, &s->infinite, 400, ClassicGenScreen_Infinite);
	AddPrimaryButton(s, &s->cancel,       Menu_SwitchPause);
...
static cc_result DoSaveMap(const cc_string* path, struct GZipState* state) {
	...
	if (World.Type == WORLD_INFINITE) {
		Chat_AddRaw("&cInfinite worlds are procedurally generated and cannot be saved.");
