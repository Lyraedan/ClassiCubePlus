...
/* 
Represents the world of blocks and associated metadata.
Can be either a finite (fixed size 3D array of blocks), or an infinite
(procedurally generated, chunk based) world.
Copyright 2014-2025 ClassiCube | Licensed under BSD-3
*/
struct AABB;
extern struct IGameComponent World_Component;

/* Type of the current world. */
enum WorldType {
	WORLD_FINITE,
	WORLD_INFINITE
};
...
CC_VAR extern struct _WorldData {
	/* Type of the world. (finite or infinite) */
	int Type;

	/* The blocks in the world. (finite mode only) */
	BlockRaw* Blocks;
...
/* Sets up an infinite, procedurally generated world and raises WorldEvents.MapLoaded event */
CC_API void World_SetInfinite(int seed);
...
static CC_INLINE BlockID World_GetBlock(int x, int y, int z) {
	...
	return (BlockID)World_GetRawBlock(World_Pack(x, y, z));
}
...
/* NOTE: For infinite worlds, all coordinates are considered inside the map. */
static CC_INLINE cc_bool World_Contains(int x, int y, int z) {
	if (World.Type == WORLD_INFINITE) return true;
